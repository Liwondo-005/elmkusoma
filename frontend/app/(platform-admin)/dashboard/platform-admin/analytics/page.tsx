...
function Kpi({ icon: Icon, label, value, sub, available }: { icon: any; label: string; value: number | null; sub: string; available: boolean }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
      ...
      {available ? <p className="mt-1 text-2xl font-bold tabular-nums">{(value ?? 0).toLocaleString()}</p> : <p className="mt-1 text-sm font-semibold text-muted-foreground">{t("analytics.dataUnavailable")}</p>}
...
export default function PlatformAnalyticsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      else setError(t("analytics.unableToLoadAnalytics"))
  ...
  async function loadSnapshots() {
    ...
    catch (e: any) { setSnapshots([]); setSnapError(e.message || t("analytics.snapshotsUnavailable")) }
  }
  ...
  async function createSnapshot() {
    ...
    try {
      ...
      await loadSnapshots()
    } catch (e: any) { setSnapError(e.message || t("analytics.failedToCreateSnapshot")) }
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-foreground"><span className="flex size-8 items-center justify-center rounded-lg bg-indigo-500 text-white"><BarChart3 className="size-4" /></span> {t("analytics.platformAnalyticsIntelligence")}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{t("analytics.realKpiCardsFrom")}<code className="rounded bg-muted px-1">getEnhancedDashboard</code>{t("analytics.noFakeChartsOnly")}</p>
          </div>
          <button onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm font-medium hover:bg-muted"><RefreshCw className="size-4" /> {t("analytics.refresh")}</button>
        </div>
      </div>
      ...
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between">
          ...
          <button onClick={load} className="rounded-lg bg-white border px-3 py-1 text-xs font-semibold">{t("analytics.retry")}</button>
        </div>
      )}

      <div>
        <h2 className="mb-3 text-xs font-bold tracking-widest uppercase text-muted-foreground">{t("analytics.keyPerformanceIndicators")}</h2>
        {loading ? (
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><History className="size-4 text-primary" /> {t("analytics.analyticsSnapshots")}</h2>
            <p className="mt-1 text-xs text-muted-foreground">{t("analytics.persistedDailySnapshotsOf")}</p>
          </div>
          <button onClick={createSnapshot} disabled={snapBusy}
            ...
            {snapBusy ? <Loader2 className="size-3.5 animate-spin" /> : <Camera className="size-3.5" />} {t("analytics.createSnapshot")}</button>
        </div>
        ...
        <div className="mt-4">
          {snapshots.length === 0 && !snapError ? (
            <div className="rounded-xl border border-dashed border-border bg-muted/30 p-6 text-center">
              <p className="text-sm font-medium">{t("analytics.noSnapshotsYet")}</p>
              <p className="mt-1 text-xs text-muted-foreground">{t("analytics.createOneNowOr")}</p>
            </div>
          ) : (
              ...
              {snapshots.map(s => (
                <div key={s.id} className="flex items-start justify-between gap-3 rounded-xl border border-border bg-card px-4 py-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{s.generatedAt ? new Date(s.generatedAt).toLocaleString() : t("analytics.dataUnavailable")}</p>
                    {s.data && (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-sm font-bold text-foreground"><TrendingUp className="size-4 text-primary" /> {t("analytics.intelligenceAttentionItems")}</h2>
          <p className="mt-1 text-xs text-muted-foreground">{t("analytics.signalsRequiringActionNo")}</p>
          <div className="mt-4">
            {loading ? <div className="h-24 animate-pulse rounded-xl bg-muted" /> : attention.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border bg-muted/30 p-8 text-center">
                ...
                <p className="mt-2 text-sm font-medium">{t("analytics.noAttentionSignals")}</p>
                <p className="mt-1 text-xs text-muted-foreground">{t("analytics.whenIncidentsVerificationsOr")}</p>
              </div>
            ) : (
              <div className="space-y-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <h2 className="text-sm font-bold text-foreground">{t("analytics.analyticsNotes")}</h2>
          <ul className="mt-3 space-y-2 text-xs leading-relaxed text-muted-foreground list-disc pl-4">
            <li>{t("analytics.allKpiValuesAre")}<code className="rounded bg-muted px-1">/v1/platform-admin/dashboard/enhanced</code>.</li>
            <li>{t("analytics.noSyntheticChartsAre")}</li>
            <li>{t("analytics.whenEndpointReturnsNo")}<span className="font-semibold">{t("analytics.dataUnavailable2")}</span> {t("analytics.perSpec")}</li>
            <li>{t("analytics.extendWithCohortOr")}</li>
          </ul>
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
            {t("analytics.chartsAreIntentionallyOmitted")}</div>
        </div>
      </div>
    </div>
  )
