...
export default function ParentLiveClassesPage() {
  ...
  const statusConfig: Record<string, { label: string; color: string }> = {
    ...
    IN_PROGRESS: { label: t("liveClasses.liveNow"), color: "bg-red-500/10 text-red-500" },
    COMPLETED: { label: ts("completed"), color: "bg-teal/10 text-teal" },
    CANCELLED: { label: ts("cancelled"), color: "bg-muted text-muted-foreground" },
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <h1 className="text-xl font-bold text-foreground">{tn("liveClasses")}</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {liveNow.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-semibold text-foreground">{t("liveClasses.happeningNow")}</h2>
                  ...
                  return (
                    <div key={lc.id} className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4 shadow-xs">
                      ...
                      <div className="mt-3 flex gap-4 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1"><Clock className="size-3" /> {t("liveClasses.durationMin", { count: lc.durationMinutes })}</span>
          ...
          {upcoming.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-semibold text-foreground">{t("assessments.upcoming")}</h2>
              <div className="space-y-3">
                {upcoming.map((lc) => {
                  const st = statusConfig[lc.status] || statusConfig.SCHEDULED
                  ...
                  let timeLabel = ""
                  if (diffDays > 0) timeLabel = t("liveClasses.inDays", { count: diffDays })
                  else if (diffHours > 0) timeLabel = t("liveClasses.inHours", { count: diffHours })
                  else timeLabel = t("liveClasses.startingSoon")
          ...
          {past.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-semibold text-foreground">{t("liveClasses.pastClasses")}</h2>
              <div className="space-y-3">
                {past.map((lc) => {
        ...
        <div className="rounded-2xl border border-dashed border-border py-12 text-center">
          ...
          <p className="text-sm font-medium text-foreground">{t("liveClasses.emptyTitle")}</p>
