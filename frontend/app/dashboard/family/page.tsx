...
const familyActivities = [
    { title: t("family.quizNight"), description: t("family.quizNightDesc"), icon: Brain, color: "text-amber-500", bg: "bg-amber-500/10" },
    { title: t("family.readTogether"), description: t("family.readTogetherDesc"), icon: BookOpen, color: "text-green-500", bg: "bg-green-500/10" },
    { title: t("family.scienceHome"), description: t("family.scienceHomeDesc"), icon: FlaskConical, color: "text-blue-500", bg: "bg-blue-500/10" },
    { title: t("family.artCraft"), description: t("family.artCraftDesc"), icon: Palette, color: "text-pink-500", bg: "bg-pink-500/10" },
    { title: t("family.musicTime"), description: t("family.musicTimeDesc"), icon: Music, color: "text-purple-500", bg: "bg-purple-500/10" },
  ]
      ...
      <div className="rounded-2xl border border-border bg-gradient-to-br from-pink-50/50 via-card to-amber-50/50 p-6 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">{t("family.title")}</h1>
          ...
          <button onClick={handleShare} className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted">
            <Share2 className="size-3.5" /> {copied ? t("family.copied") : t("family.share")}
          </button>
        </div>
      </div>
            ...
            {activities.map(a => (
              <div key={a.id} className="flex items-center gap-3 rounded-xl border border-border p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{t("together.withPartner", { name: a.partnerName })}</p>
                </div>
                {a.isCompleted && <span className="text-xs font-medium text-green-600 bg-green-100 px-2 py-0.5 rounded-full">{t("labs.doneBadge")}</span>}
              </div>
            ))}
          </div>
        </div>
