  ...
  if (!lesson) {
    return (
      <div className="mx-auto max-w-5xl p-4">
        ...
        <Link href={`/dashboard/secondary/subjects/${params.id}`} className="mt-2 text-indigo-600 underline">
          Back to subject
        </Link>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-4 pb-24">
      <div className="flex items-center gap-3">
        <Link href={`/dashboard/secondary/subjects/${params.id}`} className="flex size-10 items-center justify-center rounded-xl bg-gray-100">
          <ArrowLeft className="size-5 text-gray-600" />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold text-gray-900 truncate">{lesson.title}</h1>
          <p className="text-sm text-gray-500">Topic {currentIdx + 1} of {allLessons.length}</p>
        </div>
        {progress >= 100 ? (
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6">
        ...
        {lesson.contentText ? (
          ...
          <div className="py-8 text-center">
            ...
            <p className="mt-3 text-sm text-gray-400">Content will be added by your teacher.</p>
          </div>
        )}
        ...
        {lesson.videoUrl && (
          <div className="mt-6">
            ...
            >
              Watch Video <ChevronRight className="size-4" />
            </a>
          </div>
        )}
        ...
        {lesson.fileAttachments && (
          <div className="mt-6">
            ...
            >
              View File <ChevronRight className="size-4" />
            </a>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {nextLesson ? (
          ...
          >
            <CheckCircle className="size-4" /> Back to Subject
          </Link>
        )}
      </div>
    </div>
