  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-4 pb-24" role="main">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">{t("practicalLearning")}</h1>
          <p className="text-sm text-gray-500">{t("handsOnExperimentsAndLabWork")}</p>
        </div>
      </div>
      ...
      {selectedExperiment && activeExperiment ? (
        <div className="space-y-4">
          <button
            ...
            aria-label={t("backToExperiments")}
          >
            {t("backToExperiments")}
          </button>
          <div className="rounded-2xl border border-gray-100 bg-white p-6">
            ...
            <div className="mt-4">
              <h3 className="text-sm font-semibold text-gray-900">{t("materialsNeeded")}</h3>
            ...
            <div className="mt-4">
              <h3 className="text-sm font-semibold text-gray-900">{t("procedure")}</h3>
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 text-center">
            ...
            <h2 className="mt-3 text-lg font-bold text-gray-800">{t("labExperiments")}</h2>
            <p className="mt-1 text-sm text-gray-500">{t("chooseExperimentToBegin")}</p>
          </div>
          {experiments.map(exp => (
            <button
              key={exp.id}
              ...
              aria-label={`${exp.title} - ${exp.subject} ${exp.materials.length} ${t("materials")}`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{exp.subject} · {exp.materials.length} {t("materials")}</p>
              </div>
