...
const BUILTIN_QUESTIONS = [
  { id: "builtin-1", problemTitle: "What is the powerhouse of the cell?", options: '["Nucleus","Mitochondria","Ribosome","Cell membrane"]', correctAnswer: "Mitochondria" },
  { id: "builtin-2", problemTitle: "Simplify: 3(2x - 4)", options: '["6x - 4","6x - 12","6x - 8","3x - 12"]', correctAnswer: "6x - 12" },
  { id: "builtin-3", problemTitle: "Which planet is closest to the Sun?", options: '["Venus","Mars","Mercury","Earth"]', correctAnswer: "Mercury" },
]
...
export default function ExamModePage() {
  ...
  const allQuestions: ExamQuestion[] = [
    ...BUILTIN_QUESTIONS.map(q => {
      ...
      return { id: q.id, q: q.problemTitle, options: opts, correct: opts.indexOf(q.correctAnswer) }
    }),
    ...apiQuestions.map(p => {
      const opts: string[] = p.options ? JSON.parse(p.options) : []
      return { id: p.id, q: p.problemTitle, options: opts, correct: opts.indexOf(p.correctAnswer) }
    }),
  ]

  function handleSelect(idx: number) { if (!submitted) setSelected(idx) }
  function handleSubmit() { setSubmitted(true) }
  function getScore() { return answers.reduce((score: number, ans, i) => score + (ans === allQuestions[i]?.correct ? 1 : 0), 0) }
        ...
        <div className="mt-4 space-y-2">
          {q.options.map((opt, i) => (
            <button key={i} onClick={() => handleSelect(i)} disabled={submitted}
              ...
              <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-bold text-gray-500">{String.fromCharCode(65 + i)}</span>
