    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 pb-24">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Deep dive into key concepts</p>
        </div>
      </div>

      {selected && active ? (
        <div className="space-y-4">
          <button onClick={() => setSelected(null)} className="text-sm font-semibold text-indigo-600 hover:text-indigo-700">Back to concepts</button>
          <div className="rounded-2xl border border-gray-100 bg-white p-6">
            ...
            {active.examples && (
              <div className="mt-4 rounded-xl bg-indigo-50 p-4"><p className="text-sm font-medium text-indigo-800">Examples</p><p className="mt-1 text-sm text-indigo-700">{active.examples}</p></div>
            )}
        ...
        <div className="space-y-3">
          {allConcepts.map(concept => (
            <button key={concept.id} onClick={() => setSelected(concept.id)} className="flex w-full items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 text-left transition-all hover:border-indigo-200 hover:shadow-sm">
