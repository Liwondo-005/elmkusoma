      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="py-4 text-center text-sm text-muted-foreground">No materials found.</p>
          ) : (
              ...
              {filtered.map((material) => (
                <div key={material.id} className="flex items-center justify-between rounded-lg border border-border p-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{material.type || material.fileType || "File"}</p>
                  </div>
                  <span className="rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700">{material.status || "Available"}</span>
                </div>
              ))}
            </div>
          )}
