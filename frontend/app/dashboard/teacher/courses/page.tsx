    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-12 text-center">
          ...
          <h3 className="mt-4 text-lg font-semibold text-foreground">{t("courses.emptyTitle")}</h3>
          <p className="mt-2 text-sm text-muted-foreground">{t("courses.emptyDesc")}</p>
        </div>
      ) : (
          ...
          {classes.map((cls) => (
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><Users className="size-3" />{t("courses.students", { count: cls.enrolledStudents })}</span>
                <span className="flex items-center gap-1"><FileText className="size-3" />{t("courses.assignments", { count: cls.totalAssignments })}</span>
              </div>
            </Link>
          ))}
        </div>
