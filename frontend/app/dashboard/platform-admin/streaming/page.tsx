  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
      ...
      {value != null ? <p className="mt-1 text-2xl font-bold tabular-nums">{typeof value === "number" ? value.toLocaleString() : value}</p>
        : <p className="mt-1 text-sm font-semibold text-muted-foreground">{t("streaming.dataUnavailable")}</p>}
...
export default function PlatformStreamingPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (l.status === "fulfilled") setLive(l.value); else setError(t("streaming.liveClassTelemetryUnavailable"))
  ...
  const joinAsObserver = async (liveClassId: string) => {
    ...
    try {
      ...
      setObserverError(e.message || t("streaming.unableToJoinAs"))
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-foreground"><span className="flex size-8 items-center justify-center rounded-lg bg-violet-600 text-white"><Radio className="size-4" /></span> {t("streaming.streaming")}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{t("streaming.liveStreamingOverviewFrom")}<span className="font-semibold">{t("streaming.dataUnavailable2")}</span> {t("streaming.untilAggregatedNoFake")}</p>
          </div>
          <button onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm font-medium hover:bg-muted"><RefreshCw className="size-4" /> {t("streaming.refresh")}</button>
        </div>
      </div>
      ...
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between">
          ...
          <button onClick={load} className="rounded-lg bg-white border px-3 py-1 text-xs font-semibold">{t("streaming.retry")}</button>
        </div>
      )}
      ...
      {observerError && (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between">
          ...
          <button onClick={() => setObserverError(null)} className="rounded-lg bg-white border px-3 py-1 text-xs font-semibold">{t("streaming.dismiss")}</button>
        </div>
      )}
      {observerUrl && (
        <div className="rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-800 flex items-center justify-between gap-3">
          <span className="flex min-w-0 items-center gap-2"><Eye className="size-4 shrink-0" />{t("streaming.observerWebsocket")}<code className="truncate font-mono text-xs">{observerUrl}</code></span>
          <a href={observerUrl} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-white border border-violet-200 px-3 py-1 text-xs font-semibold hover:bg-violet-100">
            {t("streaming.open")}<ExternalLink className="size-3" />
          </a>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-bold text-foreground">{t("streaming.sessions")}</h2>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("streaming.filterByTitle")} className="rounded-xl border border-border bg-background pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </div>
          ...
          {loading ? <div className="animate-pulse space-y-2"><div className="h-14 rounded-xl bg-muted" /><div className="h-14 rounded-xl bg-muted" /></div>
            : rows.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/30 py-10 text-center">
                ...
                <p className="mt-2 text-sm font-semibold">{t("streaming.noLiveClassSessions")}</p>
                <p className="mt-1 text-xs text-muted-foreground">{t("streaming.sessionsCreatedByTeachers")}</p>
              </div>
            ) : rows.map((c) => (
              <div key={c.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-4 py-3 hover:bg-muted/30">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 flex flex-wrap gap-3 text-xs text-muted-foreground">
                    ...
                    <span>{t("streaming.min", { p0: c.durationMinutes })}</span>
                ...
                {(c.status === "LIVE" || c.status === "IN_PROGRESS") && (
                  ...
                  >
                    ...
                    {t("streaming.observe")}</button>
                )}
              </div>
            ))}
        </div>
        {all && all.totalPages > 1 && (
          <p className="mt-3 text-center text-xs text-muted-foreground">{t("streaming.showingFirstOfSessions", { p0: all.size, p1: all.totalElements })}</p>
        )}
      </div>
    </div>
  )
