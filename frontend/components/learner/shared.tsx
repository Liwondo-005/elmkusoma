...
export function LearningItemCard({ title, subject, icon }: { title: string; subject?: string; icon?: ReactNode }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border p-3 transition-colors hover:bg-muted/50" role="listitem">
      {icon && <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary" aria-hidden="true">{icon}</div>}
...
export function AssignmentCard({ title, subject, dueDate, status }: { title: string; subject?: string; dueDate?: string; status?: string }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border p-3" role="listitem">
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-foreground">{title}</p>
      ...
      {status && (
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusColors[status] || "bg-muted text-muted-foreground"}`} role="status">
          {status}
        </span>
      )}
    </div>
...
export function ProgressCard({ label, value, total }: { label: string; value: number; total?: number }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-xs" role="region" aria-label={label}>
...
export function EmptyState({ icon, title, description, action }: { icon?: ReactNode; title: string; description?: string; action?: ReactNode }) {
  return (
    <div className="rounded-2xl border border-dashed border-border py-12 text-center" role="status">
      {icon && <div className="mx-auto mb-3 flex size-10 items-center justify-center text-muted-foreground" aria-hidden="true">{icon}</div>}
