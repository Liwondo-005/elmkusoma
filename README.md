...
- **Backend:** Spring Boot `3.4.1` API (`elmkusoma-core`) plus media (`:8083`), realtime (`:8081`), and workers (`:8082`) services; PostgreSQL 16 system of record migrated by Flyway (V01–V81); ~170 JPA entities; ~67 REST controllers (~500–550 endpoints).
...
Spring Boot ──JPA──▶ PostgreSQL 16 (Flyway V01–V81)
...
| Database | PostgreSQL 16, Flyway (V01–V81), Hibernate/JPA |
...
│   │   ├── src/main/resources/          # application.yml, db/migration/ (V01–V81)
...
2. Flyway applies V01–V81 automatically on backend start (`ddl-auto: none`, `flyway.enabled: true`; prod validates).
3. Verify: `psql -U postgres -d elmkusoma -c "SELECT version FROM flyway_schema_history ORDER BY installed_rank;"` — expect V01…V81 contiguous.
...
| Flyway validation failed | Compare `flyway_schema_history` to V01–V81 files; never edit applied migrations |
...
| Database | IMPLEMENTED | ~170 entities, V01–V81 |
...
| V60 duplicate-migration concern | Flyway | RESOLVED | Single V60 file; secondary-stage migration is V65; V01–V81 contiguous, no gaps/duplicates |
...
| `docs/database/schema.md` | Pre-existing | NOT VERIFIED | Cross-check against V01–V81 migrations + entities |
